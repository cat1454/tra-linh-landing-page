...
export function ResponsiveMediaRail({
  ...
  const cards = (duplicate = false) => featured.map((item) => (
    ...
    >
      ...
      />
      <figcaption className="absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-[#07100C]/90 via-[#07100C]/60 to-transparent px-4 pb-4 pt-10 text-xs font-medium leading-snug text-[#EEF1E9]">
        {item.altText}
      </figcaption>
    </figure>
  ));
